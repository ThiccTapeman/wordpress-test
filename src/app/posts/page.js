    ...
    <section className="p-4 bg-white text-black min-h-screen">
      <div className="container mx-auto mt-30">
        <div className="flex flex-col md:flex-row justify-between items-center mb-4">
          ...
          {canEdit && (
            <ActionButton
              ...
              w="w-full md:w-max"
              additionalClasses="flex justify-center rounded-xl">
              {" "}
              Create New Post
            </ActionButton>
          )}
        ...
        <div className="grid grid-cols-1 gap-10">
          {posts.map((post) => {
            // Make the post safe to render
            const clean = DOMPurify.sanitize(
              post.content.rendered || post.content
            );
            return (
              <div
                className="p-10 px-12 bg-white rounded-xl text-black inset-shadow-sm shadow-sm"
                key={post.ID || post.id}>
                <div className="flex justify-between flex-col-reverse md:flex-row w-full">
                  <h2 className="text-xl md:text-2xl font-bold mb-1 w-full">
                    {post.title.rendered || post.title}
                  </h2>
                  {canEdit && (
                    <div className="flex gap-4 text-sm text-gray-500 relative h-max w-full md:w-max mb-5 md:mb-0">
                      <ActionButton
                        href={`/posts/edit/${post.ID || post.id}`}
                        secondary
                        w={"w-full md:w-max"}
                        additionalClasses="rounded-full h-max">
                        Edit
                      </ActionButton>
                      <ActionButton
                        href={`/posts/delete/${post.ID || post.id}`}
                        secondaryInverted
                        p="p-3"
                        additionalClasses="rounded-full hover:bg-red-400 flex justify-center items-center">
                        <Trash height={15} />
                      </ActionButton>
                    </div>
                  )}
